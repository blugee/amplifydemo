...
module.exports = {
    ...
    getLicenseCategory: async (req, res) => {
        ...
        if (isValidSubscription) {
            ...
            let licenseQuery = `select DISTINCT(licenseCategory) as items from LicenseInformation ${filters} order by licenseCategory ASC `;

            dbConn.query(licenseQuery, async (err, licenseCategory) => {
                if (err) {
                    if (err.errno === 1054) {
                    ...
                    res.json({ success: 'post call succeed!', licenseCategory: licenseCategory })
                }
            })
